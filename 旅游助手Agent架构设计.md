...
> v0.9.0 | 2026-08-10 | FastAPI + Next.js 前后端分离 | DeepSeek 驱动
...
项目定位：**展示 Agentic AI 工程能力的全栈项目**——自定义 Agent 循环、工具编排系统、中间件管道、记忆管理（v0.6.0 规则提取 + v0.9.0 向量语义）、推理反思（v0.7.0 ReAct）、自评自纠（v0.8.0 Critic）。
...
| **记忆系统**       | ✅ v0.6.0 + v0.9.0 | 双轨：规则偏好（正则 → Redis Hash）+ 向量语义（LLM 抽取 → bge-m3 嵌入 → Redis List → KNN 召回） |
...
│                   外部服务                                   │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐    │
│  │ DeepSeek API │  │ 高德地图     │  │ SiliconFlow  │    │
│  │ (LLM 推理)   │  │ (POI/路径)   │  │ (bge-m3嵌入) │    │
│  └──────────────┘  └──────────────┘  └──────────────┘    │
│  ┌────────────────┐                                       │
│  │ OpenWeatherMap │                                       │
│  │ (天气预报)     │                                       │
...
| **SiliconFlow (bge-m3)**    | 向量嵌入    | OpenAI 兼容 SDK；无 key 自动降级关闭，不阻塞主流程       |
...
处理**规则无法覆盖的非结构化事实**（"用户妈妈不吃香菜""用户上次去过成都"）——用 LLM 判断 + 抽取，bge-m3 嵌入后按语义相似度召回：

```
用户消息
  │
  ▼
① LLM 提取（memory-extract-prompt.txt，复刻 llm_classify_intent 的 prompt 组装范式）
  │   判断 should_save + 抽取 facts[]（如 "用户妈妈不吃香菜"）
  ▼
② bge-m3 批量嵌入（SiliconFlow API，OpenAI 兼容 SDK，一次批量省调用）
  ▼
③ 逐条写入 Redis List：user:memories:{user_id}  （元素 = {"text", "vector"} JSON）
  ▼
（下次对话，handle_message 入口）
④ 召回：KNN 全量余弦相似度（手写 cosine_similarity，零依赖）
  │   过滤 score ≥ MEMORY_SIM_THRESHOLD → 降序 → 截取 MEMORY_TOPK 条
  ▼
⑤ 注入 conversation.memories → PromptBuilder 拼进 System Prompt（历史记忆参考段）
```
...
| 决策 | 理由 |
| --- | --- |
| Redis List + KNN 全量遍历 | 单用户记忆条数少，手写余弦零依赖、可解释；是 **MVP 验证方案**——记忆量增长后可平滑迁移 **Redis Search**（`FT.CREATE` + 向量索引 `VECTOR HNSW`）或独立向量库，存储格式（JSON 数组）无需迁移 |
| 保存提前到意图判断前 | 所有分支（new_trip / modify_trip / ask_question / 闲聊早退）都会走到，不漏存 |
| 全链路降级 | 无 `SILICONFLOW_API_KEY` → `EmbeddingClient.available()=False` → 召回与保存全部跳过，主流程零影响；保存失败仅 WARN 日志 |
| 提取用 LLM | 非结构化事实无法用正则覆盖；LLM 判断 + 抽取更通用，代价是每消息 1 次轻量 LLM 调用 |

**双轨记忆定位**：

| 维度 | 规则偏好（4.1.1） | 向量语义（4.1.2） |
| --- | --- | --- |
| 提取 | 正则（低熵、精确、零成本） | LLM（高熵、模糊、通用） |
| 存储 | Redis Hash | Redis List |
| 召回 | 全量注入 | 相似度 top-k |
| Prompt 定位 | 必须严格遵守的硬约束 | 辅助参考的历史背景 |
...
| `search_poi`        | `poi.py`（v0.9.0）      | 城市内景点查询：名称/地址/评分/人均消费       | 高德 v3 place/text API + Redis 缓存（三层降级） |

**架构价值**：加新工具只需在 `tools/` 下新建文件、定义 `Tool` 实例、在 `ALL_TOOLS` 列表追加一行——`__init__.py`、`planner.py` 不需要任何改动。该扩展性已被 v0.9.0 的 `search_poi` 实际验证（零改动接入）。这是面试中最容易展开讲的扩展性设计。
...
| v0.9.0 | 向量语义记忆（bge-m3 嵌入 + Redis List + KNN 召回）+ 高德 POI 景点查询工具 |

### 规划中 (v1.0.0)

> **路线调整（2026-08-07）**：放弃多 Agent 协作方向（当前规模下大材小用），改为**纵深**路线——在单 Agent 内把「反思、工具、记忆、工程化」做深，每个版本都能独立讲述一个可面试的工程点。v0.9.0 已按此完成「记忆向量化 + 景点检索」纵深。
